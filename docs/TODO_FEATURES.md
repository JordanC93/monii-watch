...
## Tier 13 — App Store launch readiness

The blockers and gap-closers needed to ship Monii Watch on the
Apple App Store + Google Play. Roughly: convert PWA to native
shell, add the legal/policy surface, polish onboarding for a
cold App Store user, accessibility audit.

### #1 Capacitor iOS / Android wrappers ★ blocker
- Wrap the existing Vite build in Capacitor (preferred over
  Tauri-iOS, which was previously shelved due to build issues).
- New `ios/` and `android/` directories generated by `npx cap add ios` and `npx cap add android`.
- Capacitor plugins to wire in:
  - **Local Notifications** — bills due, deal alerts, subscription cancel reminders
  - **Filesystem** — cloud-folder-sync handle (replaces the desktop-only Tauri fs plugin path)
  - **Share** — receipt + spending image share sheet
  - **Camera** — receipt capture without the file picker
  - **Biometric Auth** — optional unlock with FaceID / fingerprint
  - **Background Fetch** — sync polling while the app is suspended
- Build pipeline: GitHub Actions workflow for iOS build →
  TestFlight upload via fastlane. Android: Play Console.
- Estimated effort: 2-3 days for first build + TestFlight; ongoing
  cost for app-store metadata, screenshots, App Privacy disclosures.
- See: docs/APP_STORE.md (to be written) for the full submission
  checklist.

### #2 Privacy policy + Terms of Service pages ★ blocker
- App Store rejects apps without a hosted privacy policy URL.
- Generate at `/privacy` and `/terms` routes inside the app, with
  a public mirror on the GitHub Pages site.
- Critical claims to make explicit:
  - "We do not collect any personal data."
  - "We do not link to your bank account."
  - "All sync is end-to-end encrypted with a key only you have."
  - "We have no servers; we cannot recover your data if you lose
    your pairing phrase."
  - "We never share, sell, or rent any user data because we don't
    have any."
- Required for both stores' submission forms.

### #3 In-app crash reporter (privacy-clean) ★ blocker
- Currently errors only land in the in-app log viewer. App Store
  reviewers expect crash diagnostics for stability triage.
- Options:
  - **Local-only crash log** (privacy-clean): expand the existing
    in-app log capture to retain the last N crashes with stack
    traces. User can review + manually email if they choose.
  - **Self-hosted Sentry** on the user's Plex box (opt-in).
  - **Public Sentry** is rejected — sends data to a third party.
- Pick option 1 for v1; spec the others in docs/.

### #4 Onboarding polish for cold App Store users
- The current welcome modal assumes some context. App Store
  users come in cold and need: "what is this," "why is it
  different," "how do I get started in 60 seconds."
- New first-run flow:
  - Splash + "what makes Monii different" (3 cards: privacy,
    free, you own your data)
  - Quick income input
  - "Pick a starter category set" (already exists)
  - "Add your first account" (already exists)
  - Sample transaction or import-from-CSV demo

### #5 Bank statement import wizard prominence
- A user from YNAB/Monarch will look for "connect your bank."
  We don't have that — by design. But the path that DOES exist
  (CSV/OFX import + receipt OCR + manual entry) needs to be
  surfaced HARD on day 1 so users don't bounce.
- New empty-state "Where do transactions come from?" card on the
  Account page when no transactions exist yet. Three buttons:
  Import CSV/OFX · Scan receipt · Add manually.

### #6 Localization (at least i18n scaffolding)
- App Store doesn't require multiple languages, but reviewers
  often test in Spanish / French / German. Currently every
  string is hard-coded English.
- Minimum: wire in `react-i18next` or similar; ship English-only
  initially with the structure to add more languages.
- Currency/date formatting already locale-aware via Intl.

### #7 Accessibility audit
- VoiceOver pass: every interactive element has aria-label or
  text content; modals trap focus; landmarks for skip links.
- Dynamic Type: respect iOS text-size setting (CSS `font: -apple-system-body` or rem-scaling).
- Color contrast: every theme passes WCAG AA on text.
- Reduced motion: every animation respects
  `prefers-reduced-motion: reduce`.
- Tab order: logical across the budget table, modals, etc.
- Existing skip-link is good; expand to per-page landmarks.

### #8 App Store + Play Store metadata
- Screenshots: at least 3 per device class (iPhone 6.7", iPad
  12.9", Android phone). Generate via Fastlane snapshot or
  manually.
- App icon: 1024×1024 + every iOS/Android variant.
- Description copy: lead with privacy, not features.
- Subtitle (30 chars iOS): "Private envelope budgeting."
- Promotional text (170 chars iOS): "No bank linking. No
  subscription. No accounts. End-to-end encrypted sync between
  your devices via a phrase only you know."
- Keywords: budgeting, envelope, ynab, money, privacy, free,
  self-hosted.
- Categories: Finance (primary), Productivity (secondary).
- Age rating: 4+ (no objectionable content).

### #9 Subscription cancel link (even without subscriptions)
- App Store review form asks for a "manage subscription" URL
  even for free apps. Point at our /help#article=tip-jar (the
  tip jar is one-time IAP, not a sub — but reviewers still ask).

### #10 TestFlight beta program
- Soft-launch via TestFlight to ~25 friends-and-family before
  public submission. Catches real issues store reviewers won't.
- Distribute via the "public TestFlight link" so beta users
  don't need to be added by Apple ID.

---

## Tier 14 — Nice-to-haves not yet shipped

QoL and feature ideas that came up in review but weren't urgent
enough for the v0.6.x wave. Roughly priority-ordered.

### #1 Better category auto-suggest
- The 80-vendor map in vendors.ts is OK but misses long tail.
- Improve heuristics: payee name word similarity (Levenshtein),
  per-user training (when a user repeatedly recategorizes a
  payee, save the override as an implicit AutoRule).
- No cloud LLM (privacy promise).

### #2 Joint household mode
- Already in Tier 99 #1 backlog. Includes per-user attribution,
  joint approval flows, "his/hers/ours" UI. Big design lift.
- Defer until users actually ask.

### #3 Free-form transaction tags
- Categories are hierarchical and synced; tags would be
  flat + multi-tag-per-txn ("vacation", "client-billable",
  "tax-deductible"). Useful for cross-cutting reporting.
- New `Transaction.tags?: string[]` field. Reports page can
  filter by tag.

### #4 Per-report CSV export
- Today only the full snapshot exports. Add one-click CSV per
  report card (Spending by Category, Year-over-Year, etc.).
- 5-min UI, 30-min implementation.

### #5 Duplicate transaction detection on manual entry
- Already runs on bulk-import. Should also run on QuickAdd —
  warn the user when adding a (vendor, amount, date) that
  already exists in the same account.

### #6 Investment lot tracking
- Tier 9 #6, deferred (project owner explicitly said "don't
  care about stocks"). Mentioned for completeness only.

### #7 PDF generation for tax-prep + bills
- Year-end tax summary PDF with deductible totals.
- Per-bill PDF for sharing with accountants.
- Browser print stylesheet already does most of this — wrap
  in a "Save as PDF" button to make it discoverable.

### #8 More dashboard widgets
- Deal alerts widget
- Recent activity widget (last 5 transactions)
- Workspace summary widget
- Notes/journal widget

### #9 Per-payee notes
- Add `Payee.notes?: string`. Useful for "this vendor refuses
  refunds" / "ask for the loyalty discount."

### #10 Chat intents for deal-tracker keywords
- "watch [item] under $[X]" → set targetItemPrice + dealKeywords
  on a category.
- "stop watching [item]" → clear them.

### #11 In-app font size setting
- Currently density (compact/comfortable/spacious) exists for row
  height. Add `Settings.fontSize: 'small' | 'default' | 'large'`
  for typography. Especially valuable on iPad and for accessibility.

### #12 Offline indicator
- The app is local-first so it's always "online" in the local
