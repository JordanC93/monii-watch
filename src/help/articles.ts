...
export const HELP_CATEGORIES: Array<{ id: HelpCategory; label: string; description: string }> = [
  ...
  { id: 'goals', label: 'Goals & savings', description: 'Track what you\'re saving for.' },
...
(red), you've overspent, and Monii nudges you to cover it from another
envelope.
...
that's $1,500 floating around with no plan, and usually it gets spent on
...
- **Statement closing day** + **Payment due day**: drive reminders
...
- **Cash on hand**: sum of liquid balances (checking, savings,
  cash). Excludes credit limits, since those are debt not runway.
...
- Transfers between your own accounts are EXCLUDED (they're not
  income or spending, just same money in a different place)
...
- Try **Verify access** to re-probe the path.
- Try **Sync now** to force-push and pull.
...
- Restoring a transaction whose account was permanently deleted
  fails because there's nothing to attach it to.
- Restoring a category whose group was deleted moves it to the
  first available group instead.
- The trash is synced across devices, so if you delete on one and
...
- **Mobile** (iOS / Android): triggers the OS share sheet, so
  pick where to send.
...
- Does **every** keyword appear in the post text? (Battlefield 6
  ...
  "Battlefield 6 launches in 2026" because the second one has no $)
...
- The bump is **additive** and never overwrites manual
...
More) are all reachable in the bottom tab bar, so you don't need to
...
way after; it only fires once per version.
...
4. Optionally pick a card network (Visa / Mastercard / Amex /
   Discover) to disambiguate when two cards end in the same
   4 digits.
5. Save.
...
They cut across categories, so a tag like \`vacation\` can show up
...
unique payee gets its own rule, which prevents bleeding between
similar names.
...
CSV files are encoded UTF-8 with proper RFC 4180 escaping, so
...
both work side-by-side. They use separate storage, so to avoid
...
`,
  ...
  // ============================================================
  // v0.7.5 — sync transport setup walkthroughs
  // ============================================================

  {
    id: 'sync-overview',
    title: 'Syncing across your devices',
    category: 'sync-privacy',
...
Open Settings, then Sync to configure them.
...
- No third-party financial services. No Plaid, no Mint, no Yodlee.
- No analytics SDKs.
- No bank credentials. You enter transactions yourself or paste
  from CSV / OFX / receipts.
...
- If your devices rarely overlap (phone here, laptop there), add a
  hub. Either Self-hosted realtime sync or Google Drive works.
- If privacy matters and you have a server, run Personal backup
  server. Your data never touches anyone else's infrastructure.

Each option has its own help article with setup steps. Open Sync
settings and click the "Step-by-step setup guide" link inside the
relevant section.
`,
  },

  {
    id: 'pairing-phrase-setup',
    title: 'Pairing phrase setup (P2P sync)',
    category: 'sync-privacy',
    tags: ['pairing', 'phrase', 'p2p', 'webrtc', 'setup', 'devices'],
    body: `
The pairing phrase is the simplest way to sync. Two or more devices
that share the same phrase form a peer-to-peer mesh and stay in
step automatically when they are both online.
...
- Sync between two phones, a laptop, an iPad, the desktop app, and
  the web app, all using the same phrase.
- Work over the internet (signaling servers help with NAT
  traversal) AND on a local network.
- Catch up changes when both devices are online together.

## What it CANNOT do

- Catch up changes when only one device is online. P2P needs at
  least one peer to talk to. If you make changes on your phone
  while your laptop is off, the laptop will not see them until
  both are open at the same time.
- That is the gap that Drive sync, Self-hosted realtime sync, and
  Personal backup server all close in different ways. Pick one of
  those if you need always-available syncing.

## Privacy

- The phrase is the encryption key. Don't share it casually.
- The signaling servers see encrypted handshake traffic only. They
  cannot read your data and don't store it.
- Anyone with the phrase can join the mesh and read or change
  your budget. Keep it private.
`,
  },

  {
    ...
    category: 'sync-privacy',
    tags: ['google', 'drive', 'oauth', 'setup', 'cloud', 'backup'],
    body: `
Google Drive sync stores an encrypted snapshot of your data in
your own Drive. Google holds the bytes but cannot read them. It
is the easiest "always available" option since you don't need a
server.

## What you need

- A Google account.
- Five minutes for the one-time OAuth setup.
- A pairing phrase (the encryption key).

The free Google Cloud project you'll create costs nothing and
isn't billed. Drive storage uses the same 15 GB quota as the rest
of your account, and a Monii snapshot is typically under a
megabyte.

## Step 1: Create a Google Cloud project

1. Visit https://console.cloud.google.com/ and sign in.
2. Click the project picker at the top. Click "New project".
   Give it any name like "monii-watch". Click Create.
3. Wait for the project to finish creating, then make sure it is
   selected in the picker.

## Step 2: Turn on the Drive API

1. In the search bar at the top of the Cloud Console, type
   "Google Drive API". Click it.
2. Click "Enable".
...
3. Fill in the App name (anything works), your email, and a
   support email. Click Save and Continue through every step
   until you reach Test Users.
4. Add your own email as a test user. This is the only required
   step. Click Save.

## Step 4: Make the OAuth client ID

1. In the left sidebar, click "Credentials".
2. Click "+ Create credentials", then "OAuth client ID".
3. Application type: "Web application".
4. Name it anything.
5. Under "Authorized JavaScript origins" click Add URI. Enter the
   address you load the app from. For the desktop app, that is
   \`tauri://localhost\`. For a PWA, the host you load it from
   (e.g. \`https://localhost:5173\` for development).
6. Under "Authorized redirect URIs" add the SAME address.
7. Click Create. Copy the Client ID. It looks like a long string
   ending in \`.apps.googleusercontent.com\`.

## Step 5: Connect Monii Watch

1. Open Settings, then Sync.
2. Set a pairing phrase if you don't have one. It is the
   encryption key.
3. Open the "Google Drive" section.
4. Paste the Client ID into the box.
5. Click "Connect Google Drive". A Google sign-in popup opens.
   Pick the account you added as a test user.
6. Approve access. The popup closes and the section shows
   "Connected".
...
`,
  ...
  {
    id: 'self-hosted-sync-setup',
    title: 'Self-hosted realtime sync setup',
    category: 'sync-privacy',
    tags: ['self-hosted', 'server', 'websocket', 'plex', 'nas', 'realtime', 'sync'],
    body: `
Self-hosted realtime sync runs a small Monii Watch sync binary on
your own server. Your devices connect over WebSocket and stay in
step in real time, even when other devices are offline. The
server keeps a copy of your encrypted state so any new device can
catch up immediately.

## What you need

- A computer that's on most of the time. Plex box, NAS, Raspberry
  Pi, home server, cloud VM, anything that runs Node.js 18+ or
  Docker.
- A network port the app can reach. Port 1234 by default.
- Optionally: a domain name and TLS certificate for use over the
  open internet. Caddy or nginx as a reverse proxy makes this
  easy.
...
1. Download the project repo or just the \`server/\` folder from
   https://github.com/JordanC93/monii-watch.
2. From inside that folder, run \`docker compose up -d\`.
3. The server starts on port 1234. Persisted data lives in
   \`./monii-data\`. Logs are visible with
   \`docker compose logs -f\`.

Edit \`docker-compose.yml\` to change the port, the persistence
path, or to add a TLS proxy.
...
2. Clone the repo or copy the \`server/\` folder.
3. From inside it, run \`npm install\`.
4. Start it with \`MONII_PERSIST_DIR=./data npm start\`. Replace
   \`./data\` with wherever you want persisted state to live.
5. Set it up as a systemd service or pm2 daemon so it restarts on
   reboot.

## Option C: Behind a reverse proxy (recommended for the open internet)

If your server is reachable from the internet, put it behind
Caddy or nginx so traffic is HTTPS and the websocket runs as
WSS. The server's \`README.md\` has copy-paste recipes for both.

## Connect Monii Watch

1. Open Settings, then Sync.
2. Set a pairing phrase if you don't have one. The phrase is the
...
Both options use the same server binary, but they're independent.
Realtime sync keeps a persistent WebSocket open so changes flow
in milliseconds. Personal backup server uploads encrypted
snapshots periodically. You can run both, just one, or pick the
one that fits.

The realtime option keeps devices in sync immediately but uses a bit more battery
and bandwidth. The backup-only option is lighter weight and works
even on slow or flaky networks.
...
`,
  ...
  {
    id: 'personal-server-backup',
    title: 'Personal backup server setup',
    category: 'sync-privacy',
    tags: ['backup', 'self-hosted', 'server', 'plex', 'nas', 'snapshot'],
    body: `
Personal backup server stores periodic encrypted snapshots of
your data on your own server. Same encryption as Google Drive,
but pointing at a box you control instead of Google's. The same
small Monii Watch sync binary handles both this and the realtime
sync option, so you only need one piece of software running.

## Why use this instead of Drive

- Your data never touches a third party.
- You decide where the bytes live.
- Useful when you already have a home server and prefer to keep
  things on it.
- No OAuth setup.

If you don't already run a server, Google Drive is easier. Pick
this option only if you actually have somewhere to host the
binary.

## What you need

- A computer that's on most of the time, with a network port the
  app can reach.
- Node.js 18+ or Docker, plus the Monii Watch sync binary from
  the project repo.

## Step 1: Run the server

The server is the same code as the Self-hosted realtime sync
option. If you already have it running, you can skip to Step 2 and
just configure it for backup.

If you don't have it running yet:

1. Download the project repo or the \`server/\` folder from
   https://github.com/JordanC93/monii-watch.
2. With Docker: from inside that folder run
   \`docker compose up -d\`.
3. Without Docker: install Node.js 18+, run \`npm install\` then
   \`MONII_PERSIST_DIR=./data npm start\`.

The server's \`README.md\` has more deployment recipes including
TLS reverse-proxy setup.

## Step 2: Turn on the backup endpoint

By default the server only does realtime sync. To enable backup,
set two environment variables:

- \`MONII_BACKUP_DIR=/path/to/backups\` tells the server where to
  store the encrypted snapshots.
- \`MONII_BACKUP_TOKEN=some-long-random-string\` is your bearer
  token. The app sends it on every request. Strongly recommended
  for any setup that isn't on a private LAN.

If you use Docker, edit \`docker-compose.yml\` to add these to
the \`environment\` block, then run
\`docker compose up -d\` again.

If you run bare Node.js:
...
The server prints the URLs it serves on startup. Look for the
"backup" line.

## Step 3: Connect Monii Watch

1. Open Settings, then Sync.
2. Set a pairing phrase if you don't have one. It is the
   encryption key.
3. Open the "Personal backup server" section.
4. Fill in the URL the server is reachable at, like
   \`http://192.168.1.10:1234\` for a LAN setup or
   \`https://backup.example.com\` for an internet-facing one.
5. Paste the token you set as \`MONII_BACKUP_TOKEN\`.
6. Optionally pick a workspace name. Leave it as "default" unless
   you run multiple budgets and want each to back up separately.
7. Click "Test connection". You should see "Connected".
8. Click "Turn on".

Repeat on every device you want backed up.

## How backups work

- Push: the app uploads a fresh encrypted snapshot a few seconds
  after every change.
- Pull: on app launch, and every minute after, the app fetches
  the latest snapshot and merges it. Other devices' changes show
  up automatically.
- History: the server keeps the most recent ten versions by
